...
export async function updateProfileInfo(data: Partial<UserData>): Promise<Customer> {
  const current = AuthorizationService.getCurrentUser();
  ...
  AuthorizationService.updateCurrentUser(response.body);
...
export async function updatePassword(
  ...
  const current = AuthorizationService.getCurrentUser();
  const currentpwd = AuthorizationService.getCurrentUser();
